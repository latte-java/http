...
public class LoadHandler implements HTTPHandler {
  ...
  private void handleCompute(HTTPRequest req, HTTPResponse res) {
    int rounds = 5000;
    String roundsParam = req.getURLParameter("rounds");
    if (roundsParam != null) {
  ...
  private void handleFailure(HTTPRequest req, HTTPResponse res) {
    ...
    res.setStatus(400);
    byte[] response = ("Invalid path [" + req.getPath() + "]. Supported paths include [/, /no-read, /hello, /file, /load, /compute, /io, /stream].").getBytes(StandardCharsets.UTF_8);
  ...
  private void handleIO(HTTPRequest req, HTTPResponse res) {
    int ms = 10;
    String msParam = req.getURLParameter("ms");
    if (msParam != null) {
  ...
  private void handleLoad(HTTPRequest req, HTTPResponse res) {
    ...
    // - See benchmarks/tomcat, benchmarks/jdk-httpserver, benchmarks/jetty, benchmarks/netty
    try (InputStream is = req.getInputStream()) {
  ...
  private void handleStream(HTTPRequest req, HTTPResponse res) {
    int size = 131072;
    String sizeParam = req.getURLParameter("size");
    if (sizeParam != null) {
      size = Integer.parseInt(sizeParam);
