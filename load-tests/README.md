...
- **lattejava-load-tests** (optional) checked out at `~/dev/lattejava/lattejava-load-tests`
...
```bash
export LATTEJAVA_LOAD_TESTS_DIR=~/dev/lattejava/lattejava-load-tests
```
...
# Just lattejava-load-tests
./run-benchmarks.sh --tool lattejava --scenarios hello --duration 10s
...
Both wrk and lattejava-load-tests are capable benchmark tools, but they have different characteristics:
...
- **lattejava-load-tests** achieves lower absolute RPS (~50-60K with 100 workers) because the Java client itself has overhead. It represents a more realistic "Java application calling an HTTP server" workload. Best for validating real-world client behavior.
