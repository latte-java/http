...
| `Upgrade` header / 101 Switching Protocols | ❌ | Not supported. The generic 101 hook was removed along with h2c-via-Upgrade (deprecated by RFC 9113); the only HTTP/2 entry paths are ALPN and h2c prior-knowledge. |
...
The Upgrade / 101 hook added by Plan B was later removed along with h2c-via-Upgrade, which RFC 9113 deprecated.)
