...
- **Copyright headers:** `HTTPServerConfiguration`, `Configurable`, and `HTTP2Settings` are FusionAuth Apache-2.0 files — preserve those headers. New files (`HTTP1Configuration`, `HTTP2Configuration`) and the relocated `HTTP2RateLimits` use the MIT `The Latte Project` header.
