...
Tracking document for RFC 9113 (HTTP/2) and RFC 7541 (HPACK) conformance. This is the always-current reference for HTTP/2 in this codebase. The dated implementation history lives in `docs/design/2026-05-05-http2-design.md`.
...
HTTP/2 traffic flows through the same accept loop as HTTP/1.1. Per-connection: one virtual thread per accepted socket, a `ProtocolSelector` decides between `HTTP1Worker` and `HTTP2Connection`, then never crosses back.
...
| h2c via `Upgrade`/101 (cleartext) | ❌ | Intentionally not supported. RFC 9113 deprecated the `Upgrade: h2c` flow in favor of prior-knowledge, so it was removed along with the generic 101 Switching Protocols hook it relied on. Cleartext HTTP/2 is reached via prior-knowledge (above); h2 over TLS is reached via ALPN. |
...
Initial server settings sent in the first SETTINGS frame after the connection preface. All 6 standard settings are configurable and sent on startup. — `HTTPServerConfigurationHTTP2Test`, `HTTP2ConnectionPrefaceTest`
...
| Response-splitting defense | ✅ | Shared choke point `HTTPTools.validateResponse`, called by `writeResponsePreamble` (h1.1) and `HTTP2OutputProtocol.commitHeaders` (h2) before anything reaches the wire. — `ResponseSplittingTest.h2RejectsBadCookieValue` / `h2RejectsBadHeaderValue` Trailer validation is deferred on both protocols (see docs/design/2026-07-08-h2-response-cookies-design.md, Out of scope). |
...
✅ Both knobs shipped. — `HTTPListenerConfiguration`

| Knob | Default | Notes |
|---|---|---|
| `enableHTTP2` | `true` (TLS only — ignored on cleartext) | Controls ALPN advertisement of `h2`. On cleartext, h2c is controlled by the prior-knowledge flag below. |
...
| h2c via Upgrade/101 | ❌ (removed — RFC 9113 deprecated) | ✅ (opt-in) | ✅ (opt-in) | ✅ | ✅ | ✅ |
...
| `org.lattejava.http.server.internal.h2.HTTP2Connection$LazyHeaderOutputStream` | 474 MB | 414 MB | −13% (per-stream, unrelated path) |
...
**Context.** Plan F option 1 (writer-thread architecture work) landed in two stacked changes per `docs/implementation/2026-05-26-h2-writer-thread-coalescing.md`. **Lever A** (`ab87ef8`): 64 KiB `BufferedOutputStream` between `HTTP2FrameWriter` and the socket — eliminates the per-frame raw-write syscall by coalescing each frame's header + payload writes and accumulating across frames until a `flush()`. **Lever B** (`d20a1d4`): replaced the writer-thread's `take() → writeFrame → flush` loop with `take() → drainTo(WRITER_BATCH_SIZE - 1) → writeFrame×N → flush`, batching up to 32 frames per flush. Both targets cited in the 2026-05-19 JFR analysis (write0 ~13% CPU, queue-AQS ~18%) directly.
...
- 2026-06/07 connection redesign and window refactor (`be7883c`, `54ba12f`): resolved the §6.5.3/§6.9.x SETTINGS-sequencing and flow-control family, but regressed §3.5/2 (the `ProtocolSelector` rework replaced GOAWAY-on-bad-preface with an HTTP/1.1 fallback whose 400 + RST h2spec cannot parse) and exposed six other deterministic failures (§5.1/11, §5.3.1/1, §5.3.1/2, §8.1/1, §8.1.2.1/3, hpack §4.2/1).
- 2026-07-06 batch-5 campaign: all remaining failures fixed (see "Closed 2026-07-06" below) — **h2spec is fully clean: 147 tests, 146 passed, 1 skipped, 0 failed**. `H2SpecHarnessTest.KNOWN_FAILING_SECTIONS` is pinned to the empty set; wire-level regression tests live in `HTTP2H2SpecBatch5Test`.

### Remaining deterministic failures

> **Resolved 2026-07-06.** All four §6.5.3/§6.9.x failures below pass after the window-management refactor (`54ba12f`) — the SETTINGS negotiation now applies the peer's `INITIAL_WINDOW_SIZE` before streams open, and window violations are detected. Kept for history.
...
> **Resolved 2026-07-06.** Re-diagnosis showed the "flakiness" was mostly missing validation, not just scheduling: §5.3.1/1, §5.3.1/2, §8.1/1, §8.1.2.1/3, and hpack §4.2/1 failed deterministically when run in isolation (the full-suite run only ever surfaced a subset per pass, which looked like racing). The genuine race — the handler's 200 beating the error frame onto the wire — remains possible in principle (open follow-up #4), but with the correct error codes and validations in place every h2spec expectation is satisfied on both sides of the race. See "Closed 2026-07-06" below.
...
- **[http2 3.5/2]** Invalid connection preface produced RST instead of GOAWAY — Fixed in `fe691cf` (`socket.shutdownOutput()` immediately after `sendGoAwayDirect` so the kernel sends FIN before the peer's trailing bytes race the close). Regressed by the `ProtocolSelector` rework (`93295c3`); re-fixed 2026-07-06 (below).

### Closed 2026-07-06 (batch 5)

All six fixes are covered by wire-level tests in `HTTP2H2SpecBatch5Test`.

- **[http2 3.5/2]** Invalid preface on a dual-protocol listener → HTTP/1.1 400 + TCP RST, which h2spec cannot parse. Two-part fix: `HTTP1Connection.closeSocketOnly` now performs a graceful teardown on error closes (`shutdownOutput`, bounded 50 ms/64 KB drain, then close — mirroring `HTTP2Connection`) so no close emits an RST; and a first-request `ParseException` on an h2c prior-knowledge listener closes silently instead of sending a 400 (the client's protocol is unknown; RFC 9113 §3.4 permits closing without a GOAWAY). Plain HTTP/1.1 listeners and later keep-alive requests still get the 400 — now readable, since the FIN follows it cleanly.
- **[http2 5.1/11]** DATA on a normally-completed stream was silently ignored: handler completion always used `deregister()` (no closed memory), so the stream materialized as forgotten-closed where `handleData` returns OK. `HTTP2HandlerDelegate` now records closed memory (`close()`) when the stream closed in both directions, yielding GOAWAY(STREAM_CLOSED); a stream whose client half is still open (early response) stays forgotten so in-flight DATA is still ignored per §6.1. `recentlyClosed` is now lock-guarded because `close()` runs on handler threads.
- **[http2 5.3.1/1, 5.3.1/2]** Self-dependency validation did not exist — `HTTP2FrameReader` discarded the priority fields of HEADERS and the entire PRIORITY payload. The reader now extracts the stream dependency into `HeadersFrame`/`PriorityFrame`; a self-dependent PRIORITY is rejected in `HTTP2Stream.handleFrame`, and a self-dependent HEADERS in `HTTP2HeaderFrameHandler.handleNewStream`/`handleTrailers` — after the HPACK decode, so the dynamic table stays synchronized. Both are stream errors of type PROTOCOL_ERROR per §5.3.1.
- **[http2 8.1/1, 8.1.2.1/3]** A second HEADERS without END_STREAM was rejected with the wrong code — `HTTP2Stream.handleHeaders` returned STREAM_CLOSED where §8.1 requires PROTOCOL_ERROR (both h2spec cases take this path; 8.1.2.1/3's trailer frame also lacks END_STREAM).
- **[hpack 4.2/1]** `HPACKDecoder` accepted a dynamic-table-size update anywhere in a header block; RFC 7541 §4.2 requires it before the first header field. The decoder now throws when fields have already been decoded, which the existing header-handler plumbing maps to GOAWAY(COMPRESSION_ERROR).

---

## Roadmap

**Status:** Phases 1–3 are complete (frame codec, HPACK, stream state machine, flow control, `HTTP2Connection` runtime, `ProtocolSelector`, ALPN, DoS limits, and both transport modes — h2-over-TLS and h2c prior-knowledge — are live; the h2c-via-Upgrade/101 mode was removed as deprecated by RFC 9113). Phase 4 is complete: **h2spec runs fully clean as of 2026-07-06** (147 tests, 146 passed, 1 skipped, 0 failed — see the Bug ledger campaign history), and gRPC interop plus JDK HttpClient round-trips are verified. Phase 5 benchmarks are published (see Performance summary).
