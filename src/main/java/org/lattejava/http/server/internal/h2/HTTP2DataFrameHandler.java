...
public class HTTP2DataFrameHandler {
  ...
  public HTTP2Result handle(HTTP2Stream stream, HTTP2Frame.DataFrame f) {
    ...
    if (f.data().length > 0) {
      // §8.1.2.6: check that the running DATA total does not exceed the declared content-length.
      if (!stream.incrementDataCount(f.data().length)) {
        return new HTTP2Result.StreamError(stream.streamId(), HTTP2ErrorCode.PROTOCOL_ERROR);
      }

      stream.consumeReceiveWindow(f.data().length);

      try {
        long timeoutMs = http2Configuration.getHandlerReadTimeout().toMillis();
        if (!pipe.offer(f.data(), timeoutMs, TimeUnit.MILLISECONDS)) {
    ...
    if (f.data().length > 0) {
      ...
      writer.enqueueOrCloseWriter(new HTTP2Frame.WindowUpdateFrame(0, f.data().length));
    }
