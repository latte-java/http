...
public sealed interface HTTP2Frame {
  ...
  record SettingsFrame(int flags, byte[] payload) implements HTTP2Frame {
    public int streamId() {
      return 0;
    }
  }

  record PushPromiseFrame(int streamId, int flags, int promisedStreamId,
                          byte[] headerBlockFragment) implements HTTP2Frame {
  }

  record PingFrame(int flags, byte[] opaqueData) implements HTTP2Frame {
    public int streamId() {
      return 0;
    }
  }
